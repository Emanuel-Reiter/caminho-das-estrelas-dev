...
public class BossSlowComboState : BossBaseState {
    ...
    public override void EnterState(BossStateManager boss) {
        ...
        maxAttackDuration = boss.helper.slowComboAnimation.length;
        attackDuration = 0.0f;
        boss.animator.CrossFadeInFixedTime("SwordComboShort", boss.helper.animationTransitionFull);
    ...
    public override void UpdateState(BossStateManager boss) {
        ...
        if (attackDuration <= maxAttackDuration * 0.2f && attackDuration >= maxAttackDuration * 0.75f) boss.helper.agent.speed = 0.00f;

        //attack the player
        if (attackDuration > maxAttackDuration * 0.375f && attackDuration < maxAttackDuration * 0.475f) {
            boss.helper.AttackPlayer();
            boss.helper.AttackPlayerClose();
        }

        if (attackDuration > maxAttackDuration * 0.685f && attackDuration < maxAttackDuration * 0.735f) {
            boss.helper.AttackPlayer();
            boss.helper.AttackPlayerClose();
        }
